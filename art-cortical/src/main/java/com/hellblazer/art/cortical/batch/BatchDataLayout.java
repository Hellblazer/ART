...
public class BatchDataLayout {
     ...
     *   <li>Dimension ≥ 2 (minimum for SIMD to apply)</li>
     *   <li>Operations are SIMD-friendly (arithmetic, comparisons)</li>
     * </ul>
     *
     * <p>Phase 1 target: batch size 64 with any reasonable dimension.
     *
     * @param batchSize number of patterns
     * @param dimension pattern dimension
    ...
    public static boolean isTransposeAndVectorizeBeneficial(
        ...
        // CRITICAL FIX: Disable SIMD on platforms with narrow vectors (e.g., Apple Silicon)
        // With only 2 lanes, Vector API overhead exceeds any potential benefit
        if (SPECIES.length() <= 2) {
            return false;
        }

        // Minimum batch size for parallelism benefit (Phase 1 target: 64)
        if (batchSize < 32) {
            return false;
        }

        // Minimum dimension to amortize transpose overhead
        // Relaxed from 64 to 2 for Phase 1 (SIMD still beneficial for small dimensions)
        if (dimension < 2) {
            return false;
        }
