...
public class Layer1SIMDBatch {
     ...
     */
    public void applyDynamicsExact(double timeStep, com.hellblazer.art.cortical.dynamics.ShuntingParameters params) {
    ...
    // NOTE: Commented out due to package-private parameter class Layer1Parameters
    // This method can be reinstated when parameter classes are made public or moved to batch package
    /*
    /**
    ...
    public static Pattern[] processTopDownBatchSIMD(Pattern[] expectations,
        ...
        batch.applyAttentionStateUpdate(params.sustainedDecayRate(), params.attentionShiftRate());

        // 2. Apply memory trace integration (SIMD - long-term persistence)
        batch.applyMemoryTraceIntegration(params.sustainedDecayRate() * 0.05, 0.3);

        // 3. Calculate priming effect (SIMD - attention + memory)
        batch.applyPrimingEffect(params.primingStrength());

        // 4. Generate apical dendrite signals for Layer 2/3 (SIMD)
        batch.applyApicalDendriteSignal(params.apicalIntegration());
        ...
        var timeStep = params.timeConstant() / 20000.0;  // Convert ms to seconds
        batch.applyDynamicsExact(timeStep, shuntingParams);

        // 6. Apply saturation (SIMD)
        batch.applySaturation(params.ceiling(), params.floor());
