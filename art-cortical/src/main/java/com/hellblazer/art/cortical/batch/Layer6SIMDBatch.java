...
public class Layer6SIMDBatch {
    ...
    // NOTE: Commented out due to package-private parameter class Layer6Parameters
    ...
    public static Pattern[] processBatchSIMD(Pattern[] bottomUpInputs, Pattern[] topDownExpectations,
        ...
        batch.applyARTMatchingRule(params.onCenterWeight(), params.modulationThreshold());

        // 2. Apply off-surround inhibition (SIMD with reduction)
        batch.applyOffSurroundInhibition(params.offSurroundStrength(), 2);  // surroundSize = 2

        // 3. Apply attentional gain modulation (SIMD)
        batch.applyAttentionalGain(params.attentionalGain());
        ...
        var timeStep = params.timeConstant() / 5000.0;  // Convert ms to seconds
