...
public class Layer5SIMDBatch {
    ...
    // NOTE: Commented out due to package-private parameter class Layer5Parameters
    ...
    public static Pattern[] processBatchSIMD(Pattern[] inputs, Pattern[] previousStates,
        ...
        batch.applyAmplificationGain(params.amplificationGain());

        // 2. Detect and apply burst firing (SIMD with masking)
        batch.applyBurstFiring(params.burstThreshold(), params.burstAmplification());

        // 3. Apply state persistence (SIMD)
        var persistence = 1.0 - params.decayRate() * 0.01;
        ...
        var timeStep = Math.min(params.timeConstant() / 10000.0, 0.01);
        batch.applyDynamicsExact(timeStep, shuntingParams);

        // 5. Apply output gain (SIMD)
        batch.applyOutputGain(params.outputGain());

        // 6. Apply output normalization (SIMD with reduction)
        batch.applyOutputNormalization(params.outputNormalization());
