...
class VectorizedBinaryFuzzyARTTest {
    ...
    private VectorizedBinaryFuzzyParameters params;
    private VectorizedBinaryFuzzyART algorithm;
    private static final Random RANDOM = new Random(42);
    
    @BeforeEach
    void setUp() {
        params = VectorizedBinaryFuzzyParameters.standard()
            .withVigilance(0.8)
            .withAlpha(0.01)
            .withBeta(1.0)
            .withParallelismLevel(1)
            .withEnableSIMD(true);
            
        algorithm = new VectorizedBinaryFuzzyART();
    }
    ...
    void testFastLearning() {
        var fastParams = params.withBeta(1.0);
    ...
    void testSlowLearning() {
        var slowParams = params.withBeta(0.5);
    ...
    void testAlphaParameter() {
        var lowAlphaParams = params.withAlpha(0.001);
        var highAlphaParams = params.withAlpha(1.0);
    ...
    void testPerformanceReset() {
        ...
        assertEquals(0, statsAfter.cacheSize());
    }
    
    @Test
    @DisplayName("Should enable/disable SIMD optimization")
    void testSIMDToggle() {
        var simdParams = params.withEnableSIMD(true);
        var noSimdParams = params.withEnableSIMD(false);
    ...
    void testInputCaching() {
        var cachingParams = params.withMaxCacheSize(10);
        var pattern = Pattern.of(1.0, 0.0, 1.0, 0.0);
    ...
    void testParameterValidation() {
        assertThrows(IllegalArgumentException.class, () ->
            VectorizedBinaryFuzzyParameters.standard().withVigilance(-0.1));
        
        assertThrows(IllegalArgumentException.class, () ->
            VectorizedBinaryFuzzyParameters.standard().withVigilance(1.1));
        
        assertThrows(IllegalArgumentException.class, () ->
            VectorizedBinaryFuzzyParameters.standard().withAlpha(-0.1));
        
        assertThrows(IllegalArgumentException.class, () ->
            VectorizedBinaryFuzzyParameters.standard().withBeta(-0.1));
    }
    ...
    void testParameterRetrieval() {
        ...
        assertEquals(params.vigilance(), retrievedParams.vigilance());
        assertEquals(params.alpha(), retrievedParams.alpha());
        assertEquals(params.beta(), retrievedParams.beta());
    }
